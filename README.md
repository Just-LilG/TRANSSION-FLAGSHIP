...
Flagship 16 has its own WebUI. Features are added **one at a time** after they are verified on a real OS 16 device. **V1.20** adds **Copy log** on Home for AI dumps. Boot/reboot animation and custom status-bar overlay upload are unchanged. Installer disables Flagship 15 if it is still active.
