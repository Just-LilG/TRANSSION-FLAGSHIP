...
Flagship 16 has its own WebUI. Features are added **one at a time** after they are verified on a real OS 16 device. **V1.15** is boot animation, reboot animation, and a custom status-bar overlay upload (no bundled icon packs). Installer disables Flagship 15 if it is still active.
