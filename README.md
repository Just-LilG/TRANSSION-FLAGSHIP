...
Flagship 16 has its own WebUI. Features are added **one at a time** after they are verified on a real OS 16 device. **V1.14** is boot animation, reboot animation, and status-bar Signal/WiFi icons (None / iOS / XOS 16 / Custom; default None). Installer disables Flagship 15 if it is still active.
