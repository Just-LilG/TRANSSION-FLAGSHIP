...
if [ -f "$MODDIR/apply_blur.sh" ]; then
  ...
  log_pfd "aod vconfig=$(cat /tr_product/etc/vconfig/com.transsion.aod/build.prop 2>/dev/null | tr '\n' ' ') extras videosr=$(getprop persist.tr_video.ai_super_resolution.support 2>/dev/null) cutepet=$(getprop ro.tr_cutepet.feature.support 2>/dev/null) outdoor=$(getprop ro.tr_outdoorboost.feature.support 2>/dev/null)"
else
