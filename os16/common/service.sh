...
if [ -f "$MODDIR/apply_blur.sh" ]; then
  ...
  log_p "aod apply: on=$(os16_cfg_bool aod_os16 true) feature=$(getprop ro.tr_aod.feature.support 2>/dev/null) doze=$(getprop ro.tr_aod.doze.brightness.feature.support 2>/dev/null) half=$(getprop ro.tr_aod.half.screen.feature.support 2>/dev/null)"
  log_p "dynamicbar apply: on=$(os16_cfg_bool dynamicbar_os16 true) bar=$(getprop ro.tr_dynamicbar.support 2>/dev/null)"
else
...
log_p "  aod_doze=$(getprop ro.tr_aod.doze.brightness.feature.support 2>/dev/null)"
...
(
  ...
  os16_apply_aod_props
  os16_apply_dynamicbar_props
    echo "[$(date '+%H:%M:%S')] blur props after settle liquidglass=$(getprop ro.tr_display.liquidglass.support 2>/dev/null) sf_disable=$(getprop persist.sys.sf.disable_blurs 2>/dev/null) aod=$(getprop ro.tr_aod.feature.support 2>/dev/null) bar=$(getprop ro.tr_dynamicbar.support 2>/dev/null)" >> "$LOG"
  if [ -f "$MODDIR/apply_120hz.sh" ]; then
    . "$MODDIR/apply_120hz.sh"
    os16_apply_game_fps_props
    os16_apply_120hz_settings
