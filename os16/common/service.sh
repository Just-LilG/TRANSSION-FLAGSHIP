...
if [ -f "$MODDIR/apply_blur.sh" ]; then
  ...
  log_p "circle apply: on=$(os16_cfg_bool circle_os16 true) cts=$(getprop ro.tr_microIntelligence.circle_to_search.feature.support 2>/dev/null) os_ai=$(getprop ro.os_ai_circle_to_search_support 2>/dev/null)"
